export default {
    ...
    mounted() {
        for (let key in this.$data.classifiers) {
            let size = this.$data.classifiers[key].defData ?
                this.$data.classifiers[key].defData.length :
                this.$data.classifiers[key].length;
            console.log(`Classifier "${key}" Successfully Loaded. ${size} record${(size > 1) ? 's' : ''} found...`);
        }
    }
}
